    ...
    fn check_node(
        ...
        if let Some(ref mut corr) = corrections {
            corr.push(crate::correction::Correction {
                start: lparen_loc.start_offset(),
                end: lparen_loc.end_offset(),
            ...
            if let Some(rparen_loc) = def_node.rparen_loc() {
                corr.push(crate::correction::Correction {
                    start: rparen_loc.start_offset(),
                    end: rparen_loc.end_offset(),
                    replacement: "".to_string(),
                    cop_name: self.name(),
                    cop_index: 0,
                });
...
mod tests {
    ...
    crate::cop_autocorrect_fixture_tests!(DefWithParentheses, "cops/style/def_with_parentheses");
}
