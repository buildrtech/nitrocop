#!/usr/bin/env python3
"""Tests for summarize_agent_result.py."""
import json
import subprocess
import sys
import tempfile
...
SCRIPT = Path(__file__).parents[2] / "scripts" / "ci" / "summarize_agent_result.py"
    ...
    with tempfile.NamedTemporaryFile(mode="w", suffix=".jsonl", delete=False) as events_file:
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".txt", delete=False) as last_file:
            ...
            result = subprocess.run(
                [sys.executable, str(SCRIPT), events_file.name, last_file.name],
                capture_output=True,
                text=True,
                check=True,
            )
