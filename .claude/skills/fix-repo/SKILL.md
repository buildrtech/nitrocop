...
4. Verify each fixed cop against the corpus:
   ...
   **Corpus validation is the acceptance gate** — unit tests passing is necessary but
   NOT sufficient. The goal is `PASS: perfect conformance` (0 FP, 0 FN). `PASS (no
   regression): N FP remain` means FPs from CI still exist — the cop is NOT fixed yet.
   Use `--rerun` only for cops changed in this batch; for untouched cops, use artifact
   mode (`--verbose`).
